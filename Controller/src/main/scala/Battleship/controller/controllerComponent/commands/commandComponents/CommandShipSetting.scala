        ...
        controller.playerState match {
          case PlayerState.PLAYER_ONE =>
            controller.requestHandleFieldSetting("player_01", coords) match {
              ...
              case None => handleShipSetFinishing(controller.player_01, PlayerState.PLAYER_TWO, GameState.SHIPSETTING)
            }
          ...
          case PlayerState.PLAYER_TWO =>
            controller.requestHandleFieldSetting("player_02", coords) match {
              case Some(exception) => publishFailure(exception.getMessage)
              case None => handleShipSetFinishing(controller.player_02, PlayerState.PLAYER_ONE, GameState.IDLE)
            }
