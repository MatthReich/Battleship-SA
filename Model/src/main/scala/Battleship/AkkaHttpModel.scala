...
object AkkaHttpModel {
  ...
  val dataBase: DaoInterface =
    injector.instance[DaoInterface](Names.named("mongodb"))
  dataBase.create()

  val grid_player_01: InterfaceGrid =
    Grid(10, new StrategyCollideNormal, Vector[Map[String, Int]]()).initGrid()
  val grid_player_02: InterfaceGrid =
    Grid(10, new StrategyCollideNormal, Vector[Map[String, Int]]()).initGrid()
  ...
  val route: Route = concat(
        ...
        entity(as[String]) { jsonString =>
          {
            ...
            (json \ "player").as[String] match {
              case "player_01" =>
                requestHandler.commandShipSetting(
                  coordsToVectorMap(json \ "coords"),
                  player_01,
                  payloadExtractGameState(json \ "gameState")
                ) match {
                  case Success(newPlayer) =>
                    player_01 = newPlayer
    ...
    path("model" / "player" / "idle" / "request") {
      parameters("gameIsWon") {
        case "player_01" =>
          if (!player_01.shipList.exists(_.status == false))
            complete(StatusCodes.OK)
          else complete(StatusCodes.BadRequest)
        case "player_02" =>
          if (!player_02.shipList.exists(_.status == false))
            complete(StatusCodes.OK)
          else complete(StatusCodes.BadRequest)
        case _ => complete(StatusCodes.BadRequest)
      }
    ...
    path("model" / "database") {
      parameters("request", "gameState".optional, "playerState".optional) { (request, gameState, playerState) =>
        request match {
          case "load" =>
