...
object AkkaHttpModel {
  ...
  val route: Route = concat(
    path("model") {
      parameters(
        "getPlayerName".optional,
        "getPlayerShipSetList".optional,
        "getPlayerGrid".optional) {
        (name, shipSetList, grid) =>
        ...
        entity(as[String]) {
          string =>
            ...
            (json \ "player").as[String] match {
              case "player_01" =>
                player_01 = player_01
                  .updateName("player_01")
                  .updateShipSetList(payloadExtractMap(json \ "shipSetList"))
                  .updateGrid(grid_player_01)
                complete(StatusCodes.OK)
              case "player_02" =>
                player_02 = player_02
                  .updateName("player_02")
                  .updateShipSetList(payloadExtractMap(json \ "shipSetList"))
                  .updateGrid(grid_player_02)
                complete(StatusCodes.OK)
              case _           => complete(StatusCodes.BadRequest)
            }
        }
    ...
    path("model" / "player" / "name" / "update") {
      parameters("playerName", "newPlayerName") {
        (player, newName) =>
          requestHandler
            .commandPlayerSetting(player, newName, player_01, player_02) match {
            ...
            case Failure(exception) => complete(
                HttpResponse(StatusCodes.custom(469, exception.getMessage)))
          }
      }
    },
        ...
        entity(as[String]) {
          jsonString =>
            ...
            (json \ "player").as[String] match {
              case "player_01" =>
                player_01.grid
                  .setField("SHIPSETTING", coordsToVectorMap(json \ "coords"))
                complete(StatusCodes.OK)
              case "player_02" =>
                player_02.grid
                  .setField("SHIPSETTING", coordsToVectorMap(json \ "coords"))
      ...
      post {
        entity(as[String]) {
          jsonString =>
            val json = Json.parse(jsonString)
            (json \ "player").as[String] match {
              case "player_01" => requestHandler.commandShipSetting(
                  coordsToVectorMap(json \ "coords"),
                  player_01,
                  payloadExtractGameState(json \ "gameState")) match {
                  case Success(newPlayer) =>
                    player_01 = newPlayer
                    complete(HttpResponse(status = StatusCodes.OK))
                  case Failure(exception) => complete(
                      HttpResponse(
                        StatusCodes.custom(469, exception.getMessage)))
                }
              case "player_02" => requestHandler.commandShipSetting(
                  coordsToVectorMap(json \ "coords"),
                  player_02,
                  payloadExtractGameState(json \ "gameState")) match {
                  case Success(newPlayer) =>
      ...
      post {
        entity(as[String]) {
          jsonString =>
            val json = Json.parse(jsonString)
            (json \ "player").as[String] match {
              case "player_01" => requestHandler.commandIdle(
                  coordsToVectorMap(json \ "coords"),
                  player_01,
                  payloadExtractGameState(json \ "gameState")) match {
                  case Left(way)        => way match {
                      ...
                      case Right(newPlayer) => // player change
                        player_01 = newPlayer
                        complete(
                          HttpResponse(StatusCodes.custom(468, "change player")))
                    }
                  case Right(exception) => complete(
                      HttpResponse(
                        StatusCodes.custom(469, exception.getMessage)))
                }
              case "player_02" => requestHandler.commandIdle(
                  coordsToVectorMap(json \ "coords"),
                  player_02,
                  payloadExtractGameState(json \ "gameState")) match {
                  case Left(way)        => way match {
    ...
    path("model" / "database") {
      parameters("request", "gameState".optional, "playerState".optional) {
        (request, gameState, playerState) =>
          request match {
            case "load" =>
              ...
              player_01 = Player(
                ...
                newPlayer1._3,
                newPlayer1._4,
                Grid(10, new StrategyCollideNormal, newPlayer1._2))
              player_02 = Player(
                newPlayer2._1,
                newPlayer2._3,
                newPlayer2._4,
                Grid(10, new StrategyCollideNormal, newPlayer2._2))
              setControllerStates(newPlayer1._5, newPlayer1._6)
              complete(StatusCodes.OK)
            case "save" =>
              val evaluatedGameState: String   = gameState match {
                case None        => ""
                case Some(value) => value
              }
              val evaluatedPlayerState: String = playerState match {
                case None        => ""
                case Some(value) => value
              }
              if (evaluatedPlayerState != "" && evaluatedGameState != "") {
                dataBase.update(
                  1,
                  player_01.name,
                  player_01.grid.grid,
                  player_01.shipSetList,
                  player_01.shipList,
                  evaluatedGameState,
                  evaluatedPlayerState)
                dataBase.update(
                  2,
                  player_02.name,
                  player_02.grid.grid,
                  player_02.shipSetList,
                  player_02.shipList,
                  evaluatedGameState,
                  evaluatedPlayerState)
                complete(StatusCodes.OK)
  ...
  def retryWithNewDatabase(database: String, gameState: String, playerState: String): Unit = {
    dataBase = injector.instance[DaoInterface](Names.named(database))
    dataBase.create()
    dataBase.update(
      1,
      player_01.name,
  ...
  private def setControllerStates(
      gameState: String,
      playerState: String): Unit = {
    val responseFuture: Future[HttpResponse] = Http().singleRequest(
      Get(
        s"http://${controllerHttp}/controller/update/states?setGameState=" + gameState + "&setPlayerState=" + playerState))
    Await.result(responseFuture, atMost = 10.second)
  }

  private def coordsToVectorMap(
      coordsJs: JsLookupResult): Vector[Map[String, Int]] = coordsJs.result.toOption match {
    case Some(value) => value.as[Vector[Map[String, Int]]]
    case None        => Vector[Map[String, Int]]()
  }

  private def payloadExtractGameState(gameStateJs: JsLookupResult): String = gameStateJs.as[String]

  private def payloadExtractMap(jsmap: JsLookupResult): Map[String, Int] = jsmap.result.toOption match {
